...
local function _processUnits()

	print "processUnits"
	print (_isRaid, _isGroup)

	if _isRaid == false and _isGroup == false then return end

	local lookupTable = {}
	local doLookup = false

	for idx = 1, 20, 1 do
		local identifier = stringFormat("group%02d", idx)
		local unit = _getIdentifier (unit)
		if unit == false then
			lookupTable[identifier] = true
			doLookup = true
		end
	end

	local checkUnits = InspectUnitLookup(lookupTable)
	local addUnits = {}
	local doAdd = fale

	for identifier in pairs(lookupTable) do
		local unitid = checkUnits[identifier] or false
		
		if unitid then
			addUnits[identifier] = unitid
			doAdd = true
		end		
	end

	if doAdd then _eventUnitAdd(_, addUnits) end

end

local function _toggleRaid (state)

	for idx = 1, 20, 1 do
		local identifier = stringFormat("raid%02d", idx)
		local frame = uiElements.frames[identifier]
		if frame then
			if state == true and data.identifierToUnit[identifier] ~= nil then
...
local function _toggleGroup (state)
	...
	for idx = 1, 5, 1 do
		...
		local frame = uiElements.frames[identifier]
		if frame then
			print (idx, identifier, data.identifierToUnit[identifier])
			local unit = data.identifierToUnit[identifier]
			if state == true and unit ~= nil then
...
local function _trackUnit (identifier, unit)	
	...
	if stringFind(identifier, 'group') then
		...
		raidIdentifier = stringFormat("raid%02d", groupId)
		frame = uiElements.frames[raidIdentifier]

		if frame then
			data.identifierToUnit[raidIdentifier] = unit
			frame:SetUnitID (unit)
			frame:ProcessUnitDetails(unit)
			frame:SetVisible(true)
			frame:ContextMenu(unit)
...
local function _untrackUnit (identifier, unit)
	...
	if stringFind(identifier, 'group') and _isRaid then
		local groupId = stringSub(identifier, 6, 7)
...
local function checkGroupSize (unitType)
	...
	print ("checkGroupSize")
	print (unitType)

	if stringFind(unitType, 'group') then return false

	_groupMembers = 0
	_raidMembers = 0

	for idx = 1, 20, 1 do
		if data.identifierToUnit[identifier] ~= nil then _raidMembers = _raidMembers + 1 end
	end
	...
	local groupId = stringSub(unitType, 6, 7)
		...
		elseif _isGroup == true then
			...
			for idx = 1, 5, 1 do
				local identifier = stringFormat('group%02d', idx)
				if data.identifierToUnit[identifier] ~= nil then _groupMembers = _groupMembers + 1 end
			end
		...
		if indicateGroupChange then

			print "indicateGroupChange"
			print (_isRaid, _isGroup)
			_toggleRaid (_isRaid)
			_toggleGroup (_isGroup)
		end
	end
...
local function _unitAvailable (_, info)        
	...
	for unit, thisData in pairs(info) do
		...
		elseif thisData ~= false then			
			_eventUnitAdd (_, {[unit] = thisData})
			checkGroupSize (thisData)
		end		    
	end
end
