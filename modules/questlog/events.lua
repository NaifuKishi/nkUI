		...
		function ()
			for idx = 1, #list, 1 do
				...
				if key ~= nil then				
					...
					if flag then
						questLog.processQuest(details, true)							
						...
						uiElements.questLog:AddQuest(key, details.domain, details.name, details.subName, details.level)
						coroutine.yield(idx)
					end
				end
			end
...
function questLog.eventInventoryUpdate(_, items)
...
function questLog.eventUnitLevel(_, units)
	...
	questLog.clearLog(questLog.fillLog)

end

function questLog.eventQuestAccept (_, quests)
...
function questLog.eventQuestAbandon (_, quests)

	for k, v in pairs (quests) do
		table.insert(data.removeQuestLog, k)
	end

end

function questLog.eventQuestChange (_, quests)
...
function questLog.eventQuestComplete (_, quests)
...
function questLog.eventSystemUpdate()
	...
	if uiElements.useUI == nil and isSecure == false then
		if LibEKL.Inventory.getAvailableSlots() ~= false then
			uiElements.useUI = questLog.buildUseUI ()
			uiElements.useUI:Update()
		end
	...
	if forceUpdate then
		...
		if #data.addQuestLog > 0 then
		
			questAdd(data.addQuestLog)
			data.addQuestLog = {}
		...
		elseif #data.removeQuestLog > 0 then

			questRemove(data.removeQuestLog)
			data.removeQuestLog = {}
...
function internalFunc.CheckCarnageNPC(npcName)
	...
	return questLog.carnageMobs[npcName]
