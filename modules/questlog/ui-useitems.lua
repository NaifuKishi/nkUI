...
local context = UI.CreateContext("nkUI.questLog.UseItem")
...
local function useItem(name, parent)
	...
	useItemTexture:EventAttach( Event.UI.Input.Mouse.Cursor.In, function ()
		questLog.showTooltip (useItem, questId, itemId, "personal", nil)
...
function questLog.buildUseUI ()

	local name = "nkUI.questLog.UseUI"

	local ui = LibEKL.UICreateFrame("nkFrame", name, context)
	ui:SetPoint("TOPRIGHT", uiElements.questLog, "TOPLEFT", 20, 35)
	ui:SetWidth(50)
	...
	function ui:Update()
		...
		if nkDebug then nkDebug.logEntry (addonInfo.identifier, "_questLog.buildUseUI", "quest items", itemList) end
