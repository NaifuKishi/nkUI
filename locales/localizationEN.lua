local addonInfo, privateVars = ...

---------- init namespace ---------

---------- init language texts ---------

if ( LibEKL.Tools.Lang.GetLanguage()  ~= "German") and ( LibEKL.Tools.Lang.GetLanguage()  ~= "French") then

	privateVars.langTexts = {
		startUp             	= '<font color="#0094FF">nkUI</font> V%s loaded',
		commandline1            = '/nkui to open settings',
		commandline2            = '/nkui qlog to open the quest log',
		commandline3            = '/nkui bag to open the inventory',
		questGiver				= 'Quest giver: <font color="#0094FF">%s</font>',
		scene					= 'Quest location: <font color="#0094FF">%s</font>',
		zoneFilter				= "Click here to filter the list by the current zone",
		categoryFilter			= "Click here to filter to select which categories to show",
		questItems				= "Click here to hide or show the quest items",
		track					= "Track quest",
		abandon					= 'Abandon',
		share					= 'Share',
		abandonQuestConfirm		= 'Are you sure that you want to abandon the quest <font color="#0094FF">%s</font>?',
		abandonAllQuestsConfirm = 'Are you sure that you want to abandon all quests of category <font color="#0094FF">%s</font>?',
		completeInfo			= '%s <font color="#FF0000">(done)</font>',
		categoryHeaderSize		= 'Category title size %d',
		abandonAll				= 'Abandon all',
		identifierCarnage		= 'Carnage',
		battlePass				= "Battle Pass",
		nkUISubTitle			= "A modern style UI compilation for RIFT",
		lowerBar				= { social = "Friends %d | Guild %d",
									role = "Active role: <font color='#3C99D7'>%s</font>",
									vitality = "<font color='#22B357'>%d%%</font> Vitality"},									
		oneBag					= { itemValue = "Item value: %s",
									currencyTextPlatinum = '%d<font color="#efebff"> platinum</font>',
									currencyTextGold = '%d<font color="#eed234"> gold</font>',
									currencyTextSilver = '%d<font color="#a7aba7"> silver</font>',
									noPrice = "No price information",
									bagTitle = "%s's inventory",
									bankTitle = "%s's bank",
										lastSeenPrice = "Last: %s",
																		noLastPrice = "No last price",
									noAuction = "No known auction price",
									auctionPrice = "%s (%s)",
									youOwn	= "You own: %d",
								today = "today",
								yesterday = "1 day ago",
								daysAgo = "%dd ago",
								},
		auction					= { scanStarted = "SCANNING: All auctions (initiating)....",
									scanProgress = "SCANNING: %d%%...",
									sellTitle = "Sell",
									colItem = "ITEM",
									colPrice = "PRICE",
									btnScan = "Scan",
									neverScanned = "Not yet scanned",
									notAtAH = "Open the Auction House first.",
								settingsScanHeader = "Scan",
								settingsScanDepth = "History depth <font color='#3399FF'>%d days</font>",
								settingsTrendThreshold = "Trend threshold <font color='#3399FF'>%d%%</font>",
								settingsPriceHeader = "Pricing",
								settingsPriceFloor = "Price floor <font color='#3399FF'>%d%% of avg</font>",
								settingsUndercut = "Undercut by <font color='#3399FF'>%d copper</font>",
								tooltipSuggest    = "Suggested",
								tooltipLastLo     = "Last min",
								tooltipAvg        = "Avg (days)",
								tooltipHi         = "High",
								tooltipSeen       = "Auctions seen",
								tooltipChartLabel = "PRICE HISTORY",
								tooltipNoData     = "Not enough data",
								},
		showCategoryCheckbox	= {	battlepass = "Battle Pass",
									crafting = "Crafting", 
									weekly = "Weekly", 
									monthly = "Monthly",
									daily = "Daily", 
									guild = "Guild", 
									ia = "Instant Adventure", 
									world = "World", 
									zone = "Zone", 
									area = "Area", 
									instant = "Dungeon", 
									raid = "Raid", 
									personal = "Regular quests", 
									carnage = "Carnage", 
									story	= "Story",
									pvp="PvP"},
		settings				= { activateModule = "Activate this module",
									player = "Player",
									target = "Target",
									targetOfTarget = "Target of Target",
									playerPet = "Player Pet",
									focus = "Focus",
									group = "Group",
									raid = "Raid",
									combatAlpha = "Combat transparency <font color='#3399FF'>%d</font>%%",
									nonCombatAlpha = "Non-combat <font color='#3399FF'>%d</font>%%",
									numberOfMainBars = "Number of main bars <font color='#3399FF'>%d</font>",
									activateRightBar = "Show right side bar",
									iconSize = "Icon size <font color='#3399FF'>%d</font>",
									sizeSetup = "Size setup",
									buffSize = "Buff size <font color='#3399FF'>%d</font>",
									comboPointSetup = "Combo point setup",
									timerSize = "Timer size <font color='#3399FF'>%d</font>",
									stackSize = "Stack size <font color='#3399FF'>%d</font>",
									castBarSize = "Castbar size",
									width = "Width <font color='#3399FF'>%d</font>",
									height = "Height <font color='#3399FF'>%d</font>",									
									spellName = "Spellname <font color='#3399FF'>%d</font>",
									timer = "Timer <font color='#3399FF'>%d</font>",
									fontSizes = "Font sizes",
									textDisplay = "Text display <font color='#3399FF'>%d</font>",
									timeDisplay = "Time display <font color='#3399FF'>%d</font>",
									dateDisplay = "Date display <font color='#3399FF'>%d</font>",
									expNotoriety = "Experience and notority datasets",
									textSize = "Text size <font color='#3399FF'>%d</font>",
									barHeight = "Bar height <font color='#3399FF'>%d</font>",
									barWidth = "Bar width <font color='#3399FF'>%d</font>",
									trackerSize = "Tracker size",
									width = "Width <font color='#3399FF'>%d</font>",
									height = "Height <font color='#3399FF'>%d</font>",
									fontSizes = "Font sizes",
									headerFontSize = "Header Font Size <font color='#3399FF'>%d</font>",
									categoryHeaderSize = "Category Header Size <font color='#3399FF'>%d</font>",
									subHeaderFontSize = "SubHeader Font Size <font color='#3399FF'>%d</font>",
									bodyFontSize = "Body Font Size <font color='#3399FF'>%d</font>",
									ressourceBarSize = "resource bar size",
									width = "Width <font color='#3399FF'>%d</font>",
									height = "Height <font color='#3399FF'>%d</font>",
									comboPointSize = "Combo point size",
									chargeDisplaySize = "Charge display size",
									textSizeHeader = "Text sizes",
									charge = "Charge <font color='#3399FF'>%d</font>",
									ressource = "Resource <font color='#3399FF'>%d</font>",
									showXPGains = "Show XP gains",
									showLootGains = "Show Loot gains",
									showLogo = "Show nkUI logo",
									minimapIconFrame = "Minimap icon frame",
									selectColoringMode = "Select coloring mode",
									unitFrameSize = "Unit frame size",
									width = "Width <font color='#3399FF'>%d</font>",
									height = "Height <font color='#3399FF'>%d</font>",
									reverseDisplay = "Reverse display",
									unitName = "Unit name <font color='#3399FF'>%d</font>",
									healthText = "Health text <font color='#3399FF'>%d</font>",
									energyText = "Energy text <font color='#3399FF'>%d</font>",
									planarText = "Planar text <font color='#3399FF'>%d</font>",
									levelText = "Level text <font color='#3399FF'>%d</font>",
									iconSizes = "Icon sizes",
									roleIcon = "Role icon <font color='#3399FF'>%d</font>",
									combatMarkIcon = "Combat / Mark icon <font color='#3399FF'>%d</font>",
									tierIcon = "Tier icon <font color='#3399FF'>%d</font>",
									buffDisplaySetup = "Buff display setup",
									buffIconSize = "Buff icon size <font color='#3399FF'>%d</font>",
									timerSize = "Timer size <font color='#3399FF'>%d</font>",
									stackSize = "Stack size <font color='#3399FF'>%d</font>",
									activateModule = "Activate module",
									showBuffs = "Show buffs and debuffs",
									buffDuration = "Only show buffs with duration",
									lessThan = "less than %d sec",
									maxBuffs = "Max buffs %d",
									onlyOwnBuffs = "Show only own buffs",
									smoothAnimations = "Smooth animations",
									close = "Close",
									moveUI = "Move UI",
									theme = "Basic",
									map = "Map",
									questTracker = "Quest Tracker",
									actionBar = "Action bar",
									lowerBar = "Lower bar",
									sct = "SCT",
									tooltip = "Tooltip",
									buffBar = "Buff bar",
									oneBag = "OneBag",
									ressourceBar = "Resource bar",
									playerCastbar = "Player castbar",
									targetCastbar = "Target castbar",
									unitframes = "Unitframes",
									units = "Units",
									useBank = "Bank module",
									transparent = "Transparent",
									showChat = "Chat background",
									alwaysShowRessourceBar = "Always show",
									showSCTCombat = "Show damage & heal",
									showSCTCooldowns = "Show Cooldowns",
									oneBagScale = "OneBag size <font color='#3399FF'>%d%%</font>",
									buffBarGrowRight = "Grow right",
									minionManager = "Minion Manager",
									auction = "Auction",
								},
		map = {	tabHeaderSettings	   			= 'Settings',
				tabHeaderAbout		   			= 'About',
				btClose				       		= 'Close',
				labelGenericSettings 			= 'Generic settings',
				labelDisplaySettings 			= 'Display settings',
				labelTrackSettings   			= 'Track settings',
				backgroundSelect     			= 'Border',
				lockedCheckbox       			= 'Locked',
				syncTargetCheckbox   			= 'Sync enemies',
				txtNone              			= 'None',
				poiCheckbox          			= 'Points of interest',
				zoneTitleCheckbox    			= 'Zone name',
				animationsCheckbox   			= 'Animations',
				animationsCheckboxheckboxInfo   = 'WARNING: performance intensive!',
				animationSpeedSlider 			= "Update frequency <font color='#3399FF'>%d%%</font>",
				iconSize             			= "Element size <font color='#3399FF'>%d</font>",
				rareCheckbox         			= 'Rare Mobs',
				rareCheckboxInfo     			= 'Requires RareDar or RareTracker addon',
				questCheckBox        			= 'Quests',
				unknownCheckbox      			= 'Unknown',
				gatheringCheckbox    			= 'Track gathering',
				artifactCheckbox     			= 'Track artifacts',				
				thanxLabel           			= 'Specials thanks go to:',
				thanxTesting         			= 'gordi@zaviel who\'s a great help getting my',
				thanxTesting2        			= 'addons bug free',
				thanxLibs            			= 'Ivnedar@Laethys for LibTransform2 and advise',				
				poiAchievement       			= 'Achievement',
				poiPuzzle            			= 'Puzzle',				
				startUp              			=  '<font color="#0094FF">nkCartographer</font> V%s loaded\n/nkCG toggle - switch between map sizes\n/nkCG show - show / hide map\n/nkCG add x y title- Add custom mark\n/nkCG clear - Clear custom marks',
				errorAddCommand		 			= 'Invalid command parameters for <font color="#0094FF">add</font> - Use /nkCG add x y title',
				toggle				 			= 'Toggle size',
				configuration		 			= 'Configuration',
				showhide			 			= 'Show / hide map',				
				waypointDialogTitle	 			= 'Waypoint',
				coordLabel			 			= 'Please enter the coordinates:',
				btSet				 			= 'Set',
				questCarnage		 			= "Carnage:"
		},
		manual = {
					windowTitle = "nkUI Manual",
					buttonLabel = "Manual",
					sections = {
						{
							title = "Welcome",
							body = string.format("Welcome to nkUI Version %s\n\nThank you for using nkUI, a comprehensive UI suite for RIFT.\n\nThis manual guides you through all nkUI modules and features.\n\nLatest Changes (v1.3.1):\n- Keybind labels for action bar icons\n- Improved cooldown tracking display\n- Better rarity filtering in Auction House Browse\n- Bug fixes in unit frame rendering\n\nClick the sections on the left to explore each module.\n\nFor quick access: /nkui (settings) | /nkui qlog (quest log) | /nkui bag (inventory)", addonInfo.toc.Version)
						},
						{
							title = "Overview",
							body = "Welcome to the nkUI Manual.\n\nnkUI is a complete UI suite for RIFT, replacing or enhancing most default UI elements with a modern, cohesive design.\n\nUse the list on the left to jump to any module. Each section explains what the module does and how to use it.\n\nYou can open the settings at any time with /nkui, open the Quest Log with /nkui qlog, the bag with /nkui bag."
						},						
						{
							title = "Default UI Setup",
							body = "Before using nkUI, you need to hide the default RIFT UI elements to avoid overlap.\n\nHow to hide default UI elements:\n1. Press Escape during gameplay to open the Settings menu\n2. Select [Edit Layout]\n3. A layout editor window will appear\n4. Uncheck or hide the following default elements:\n   - Quest Log (we replace this)\n   - Quest Tracker (we replace this)\n   - Default action bars (if using nkUI action bars)\n   - Default unit frames (if using nkUI unit frames)\n   - Buff/Debuff frame (if using nkUI buff bar)\n   - Minimap (if using nkUI map)\n\n5. Click [Done] to save your layout\n6. The nkUI elements will now be fully visible without overlap\n\nNote: The RIFT API prevents addons from hiding these elements automatically, so this manual step is necessary and only needs to be done once per character."
						},
						{
							title = "Theme & Basic",
							body = "The Basic / Theme tab in Settings controls global appearance options.\n\nOptions:\n- Show or hide the nkUI logo in the corner\n- Show or hide the minimap manager icon\n- Enable or disable the chat background\n- Select the color scheme: RIFT (class colors by soul) or WoW-style coloring\n- Adjust the OneBag window scale\n\nThese settings apply globally across all nkUI modules."
						},
						{
							title = "Move UI",
							body = "The Move UI mode lets you reposition all nkUI elements on your screen.\n\nHow to use:\n1. Open Settings (/nkui)\n2. Click the 'Move UI' button\n3. Drag any highlighted frame to your desired position\n4. The current coordinates are shown while dragging\n5. A grid overlay helps with alignment\n6. When done, the Settings window reappears automatically\n\nMovable elements include: all Unit Frames, Castbars, Resource Bar, Action Bars, and the Buff Bar.\n\nPositions are saved per account and persist across sessions."
						},												
						{
							title = "Map",
							body = "The Map module is a full replacement for the default RIFT map.\n\nFeatures:\n- Displays your current zone name and coordinates\n- Supports Points of Interest (POI) markers\n- Rare mob tracking (requires RareDar or RareTracker addon)\n- Quest markers on the map\n- Records and displays gathering nodes (ore, wood, etc.) and artifact locations\n- Smooth animated zoom\n- Toggle between minimap and full-screen mode with /nkui map toggle\n- Lock the map position to prevent accidental movement\n\nTip: You can add custom markers from the map settings. Unknown zones can optionally be shown."
						},
						{
							title = "Quest Log",
							body = "The Quest Log module replaces the default RIFT quest journal.\n\nOpen it with: /nkui qlog\n\nFor convenience, create a macro with '/nkui qlog', place it on an action bar slot, and bind the L key to that slot.\n\nFeatures:\n- Organized by quest category (Story, Daily, Weekly, World, Crafting, Raid, etc.)\n- Click a quest to read its full description, objectives, and rewards\n- Abandon or share quests directly from the log\n- Abandon all quests in a category at once"
						},
						{
							title = "Quest Tracker",
							body = "The Quest Tracker module replaces the default on-screen quest tracker.\n\nFeatures:\n- Shows all active quests with live objective progress\n- Scroll with the mouse wheel when many quests are tracked\n- Click the 'C' button in the header to choose which quest categories to display\n- Click the 'Z' button to filter quests to your current zone only\n- Click the 'I' button to show quest items\n- Right-click a quest item button to use the item directly from the tracker\n- Font sizes and window size are configurable in Settings"
						},
						{
							title = "Unit Frames",
							body = "The Unit Frames module provides a modern replacement for the default health and energy bars.\n\nDisplayed frames:\n- Player, Player Pet, Target, Target of Target, Focus, Group, Raid\n- Player and Target Castbars\n- Resource Bar (combo points, charges, etc.)\n\nFeatures:\n- Health, energy, and planar bars with smooth animations\n- Role icon, combat icon, and tier badge on each frame\n- Buff and debuff display with timer and stack count\n- Separate alpha settings for combat and out-of-combat\n- Choose between RIFT color scheme (class colors) or WoW-style coloring\n- All frames can be repositioned via Settings > Move UI\n\nNote: Configure each frame individually in Settings > Units."
						},
						{
							title = "Action Bar",
							body = "The Action Bar module provides themed action bars that match the nkUI visual style.\n\nFeatures:\n- Up to 4 main horizontal bars and 1 vertical side bar\n- Drag and drop abilities, items, and macros onto slots\n- Cooldown timers shown on each slot\n- Out-of-range indicator (slot dims when target is too far)\n- Middle-click a slot to open the Macro Editor and assign a custom macro\n- Keybind labels: Right-click a slot to assign a keybind label that appears on the icon\n- Use the keybind dialog to clear a slot\n- Combat and non-combat transparency are independently configurable\n\nImportant: Due to RIFT API limitations, actual key bindings must be set up on the default RIFT action bars. Set your abilities there, hide those bars, and use nkUI bars for display and click interaction."
						},
						{
							title = "Lower Bar",
							body = "The Lower Bar module displays an information bar at the bottom of the screen.\n\nDisplayed information:\n- Current date and time\n- Your active role\n- Current zone and coordinates\n- Frames per second (FPS) and latency\n- Currency (platinum, gold, silver)\n- Current experience and notoriety progress bars\n- Vitality percentage\n- Friends and guild member online count\n\nThe bar can be set to transparent mode, and font sizes and bar dimensions are adjustable in Settings.\n\nInteractive elements:\n- Click on the location to show teleport options\n- Click on the role to switch to another role\n- Click on the CPU/FPS display to open nkDebug if installed"
						},
						{
							title = "OneBag",
							body = "The OneBag module replaces the default bag system with a single unified inventory window.\n\nOpen it with: /nkui bag\n\nTip: Create a macro with '/nkui bag', place it on a hidden default action bar slot, and bind the B key to that slot.\n\nFeatures:\n- All bag slots displayed in a single window\n- Bank window also available (/nkui bag will open the bank when at a banker)\n- Items color-coded by category\n- Shows item value (vendor price) and last known auction price\n- Configurable scale in Settings\n\nNote: Set your default in-game bags to 50% scale to minimize visual clutter."
						},
						{
							title = "Buff Bar",
							body = "The Buff Bar module shows your active buffs and debuffs as icon rows near the edge of the screen.\n\nFeatures:\n- Separate rows for buffs and debuffs\n- Each icon shows remaining duration and stack count\n- Icon size, timer font size, and stack font size are configurable\n- The bar can grow to the right or left\n- Position is adjustable via Settings > Move UI"
						},
						{
							title = "Scrolling Combat Text",
							body = "The Scrolling Combat Text (SCT) module replaces the default on-screen combat messages.\n\nFirst-time setup: Disable the default SCT in RIFT settings under Settings > Interface > Screen Messages.\n\nDisplayed messages:\n- Damage dealt and received\n- Healing done\n- Experience and notoriety gains\n- Loot received\n- Ability cooldown completions\n\nEach message type can be individually enabled or disabled in Settings > SCT."
						},
						{
							title = "Tooltip",
							body = "The Tooltip module reskins unit to match the nkUI style.\n\nFeatures:\n- Clean, styled tooltip frame for units and items\n- Header and body font sizes are configurable in Settings\n\nNote: Due to RIFT API limitations, quest information for NPCs cannot be displayed in tooltips."
						},
						{
							title = "Chat",
							body = "The Chat module adds a styled background to the default RIFT chat window.\n\nFeatures:\n- Semi-transparent background frame behind the chat box\n- Can be enabled or disabled in Settings > Basic\n\nNote: The chat window itself (position, channels, font) is still managed by the default RIFT UI."
						},
						--[[{
							title = "Cooldowns",
							body = "The Cooldowns module tracks active ability and item cooldowns and displays them as a list.\n\nFeatures:\n- Shows all currently cooling down abilities as a vertical bar list\n- Each entry has an icon, a progress bar, and a countdown timer\n- Cooldowns are sorted by remaining time\n- Automatically removes entries when cooldowns finish\n\nThis module runs in the background and requires no configuration."
						},]]

					}
				},
		itemCategories = {
					various = "Various",
					consumable = "Consumable",
					armor = "Armor",
					artifact = "Artifact",
					quest = "Quest",
					fishing = "Fishing",
					meat = "Meat",
					butchering = "Butchering",
					cloth = "Cloth",
					weapon = "Weapon",
					trash = "Trash",
					craftingMaterial = "Crafting material",
					craftingRecipe = "Crafting recipe",
					container = "Container",
					costume = "Costume",
					dimension = "Dimension",
					planarFocus = "Planar Focus",
					planarLesser = "Planar Lesser",
					planarGreater = "Planar Greater"
				},
		questLog = {
			rewards = "REWARDS",
			objectives = "OBJECTIVES",
			detailedDescription = "DETAILED DESCRIPTION",
			guaranteedRewards = "GUARANTEED REWARDS",
			chooseableRewards = "CHOOSEABLE REWARDS",
			unknownFaction = "Unknown Faction",
			reputationWith = "Reputation with <font color=\"#3ca7dd\">%s</font>",
			suffixList = {
				exp = "Experience",
				prestige = "Prestige",
				favor = "Favor",
				guildExp = "Guild Experience"
			}
		},
		minionManager = {
				windowTitle      = "Minion Manager",
				available        = "Available Minions",
				adventures       = "Adventures",
				active           = "Active Missions",
				send             = "Send",
				sendNow          = "SEND",
				claim            = "Claim",
				done             = "Done!",
				noMinion         = "Select a minion first",
				selectedHint     = "Minion selected — click SEND NOW",
				shuffle          = "Shuffle",
				slotsLabel       = "%d Slot(s)",
				noAdventure      = "No adventure",
				bottomBarHint    = "Click a minion, then SEND NOW on an adventure card",
				selectAdventure  = "Select an Adventure",
				selectMinion     = "Select a Minion",
				hurry            = "Hurry",
				hurryDialogMsg   = "Hurry this adventure?",
				hurryAventurine  = "Aventurine (%d)",
				hurryCredit      = "Credits (%d)",
				hurryCancel      = "Cancel",
				currencyHint     = "%d",
			autoSend               = "AUTO",
			autoSendHeader         = "Auto Send Settings",
			autoSendNoAdventure    = "Auto Send: no adventure found for the configured duration tier",
			autoSendNoMinion       = "Auto Send: no eligible minion found",
			autoSendDurationLabel  = "Duration tier",
			autoSendDuration1      = "Quick (< 5 min)",
			autoSendDuration2      = "Short (5-20 min)",
			autoSendDuration3      = "Long (> 20 min)",
			autoSendDuration4      = "Premium (costs currency)",
			autoSendPriorityLabel  = "Priority",
			autoSendPriority1      = "Best stats",
			autoSendPriority2      = "Lowest level",
			autoSendPriority3      = "Highest level",
			autoSendPriority4      = "Distribute stamina",
			autoSendMatchLabel     = "Stat matching",
			autoSendMatch0         = "None required",
			autoSendMatch1         = "Match 1+ stats",
			autoSendMatch2         = "Match 2+ stats",
			},
		wardrobe = {
			windowTitle = "Wardrobe",
			setLabel = "Set name",
			activeSet = "Active set: %d - %s",
			tabHeaderSets = "Sets",
			partOfSets = "Part of set(s): ",
			loadSet = 'Loading nkUI wardrobe set <font color="#0094FF">%s</font>',
			errGettingItem = 'Cannot access or find %s',
			errMovingItem = 'Problem moving %s / %s',
			errLastSet = 'The last set cannot be deleted!',
			msgRemoveSetConfirm = 'Do you really want to delete the set?',
			msgLoadCurrentEquip = 'Do you want to load the current equip into the current set?',
			msgBankWarning = "WARNING: Rift will put your items onto the bank, even if no bank is in reach!\n\nHowever in order to get equipment from your bank, you'll need to be near one.\n\nAre you sure, that you want to continue?",
			noItemsMessage = "Looks like you're naked.",

			btWearTitle = 'Equip set',
			btWearLines = 'Click this button to equip the items of this set',
			btNewSetTitle = 'New set',
			btNewSetLines = 'Create a new, empty set',
			btDeleteSetTitle = 'Delete set',
			btDeleteSetLines = 'Deletes the set permanently',
			btCopySetTitle = 'Copy set',
			btCopySetLines = 'Creates a copy of the current set',
			btLoadGear = "Current equipment",
			btLoadGearTitle = 'Get current equipment',
			btLoadGearLines = 'Gets the current equipment and stores it into the selected set.',

			txtEquipment = 'Set %d',
			fldIcon = 'Icon',
			fldBagSelect = "Inventory bag",
			fldBankSelect = "Bank slot",
			txtBag = 'Bag %d',
			mainBankSlot = 'Main bank slot',
			txtBank = 'Bank slot %d',
			btGetFromBank = 'Get from bank',
			btPutToBank = 'Store into bank',
			btPutToBankTitle = 'Store into bank',
			btPutToBankLines = 'Moves the equipment of the current set to the bank.',
			btGetFromBankTitle = 'Get from bank',
			btGetFromBankLines = 'Retrieves the equipment of the current set from the bank.',

			txtEquipSlotsUIHeader = "Equipment of the set",
			txtInventorySlotsUIHeader = "Inventory",

			txtCopiedSet = 'copy of %s',

			lowerBarLabel = "Set: <font color='#3C99D7'>%s</font>",
			startUp = '<font color="#0094FF">nkUI Wardrobe</font> initialized',

			souls = {
				warrior = { {label = 'Liberator', value = 'soul_liberator.dds'},
							{label = 'Beastmaster', value = 'soul_beastmaster.dds'},
							{label = 'Champion', value = 'soul_champion.dds'},
							{label = 'Warlord', value = 'soul_warlord.dds'},
							{label = 'Paladin', value = 'soul_paladin.dds'},
							{label = 'Paragon', value = 'soul_paragon.dds'},
							{label = 'Reaver', value = 'soul_reaver.dds'},
							{label = 'Riftblade', value = 'soul_riftblade.dds'},
							{label = 'Void Knight', value = 'soul_voidknight.dds'},
							{label = 'Warchanter', value = 'soul_warchanter.dds'},
							{label = 'Tempest', value = 'soul_tempest.dds'} },
				cleric =  { {label = 'Oracle', value = 'soul_oracle.dds'},
							{label = 'Warden', value = 'soul_warden.dds'},
							{label = 'Druid', value = 'soul_druid.dds'},
							{label = 'Inquisitor', value = 'soul_inquisitor.dds'},
							{label = 'Cabalist', value = 'soul_cabalist.dds'},
							{label = 'Purifier', value = 'soul_purifier.dds'},
							{label = 'Justicar', value = 'soul_justicar.dds'},
							{label = 'Shaman', value = 'soul_shaman.dds'},
							{label = 'Sentinel', value = 'soul_sentinel.dds'},
							{label = 'Defiler', value = 'soul_defiler.dds'},
							{label = 'Runeshaper', value = 'soul_runeshaper.dds'} },
				mage = { {label = 'Arbiter', value = 'soul_arbiter.dds'},
							{label = 'Archon', value = 'soul_archon.dds'},
							{label = 'Dominator', value = 'soul_dominator.dds'},
							{label = 'Chloromancer', value = 'soul_chloromancer.dds'},
							{label = 'Elementalist', value = 'soul_elementalist.dds'},
							{label = 'Warlock', value = 'soul_warlock.dds'},
							{label = 'Necromancer', value = 'soul_necromancer.dds'},
							{label = 'Pyromancer', value = 'soul_pyromancer.dds'},
							{label = 'Stormcaller', value = 'soul_stormcaller.dds'},
							{label = 'Harbringer', value = 'soul_harbringer.dds'},
							{label = 'Frostkeeper', value = 'soul_frostkeeper.dds'} },
				rogue =  { {label = 'Physician', value = 'soul_physician.dds'},
							{label = 'Assassin', value = 'soul_assassin.dds'},
							{label = 'Bard', value = 'soul_bard.dds'},
							{label = 'Bladedancer', value = 'soul_bladedancer.dds'},
							{label = 'Nightblade', value = 'soul_nightblade.dds'},
							{label = 'Riftstalker', value = 'soul_riftstalker.dds'},
							{label = 'Saboteur', value = 'soul_saboteur.dds'},
							{label = 'Marksman', value = 'soul_marksman.dds'},
							{label = 'Ranger', value = 'soul_ranger.dds'},
							{label = 'Tactician', value = 'soul_tactician.dds'},
							{label = 'Shadeborn', value = 'soul_shadeborn.dds'} },
				primalist = {{label = 'Vulcanist', value = 'soul_vulcanist.dds'},
							{label = 'Berserker', value = 'soul_berserker.dds'},
							{label = 'Dervish', value = 'soul_dervish.dds'},
							{label = 'Preserver', value = 'soul_preserver.dds'},
							{label = 'Titan', value = 'soul_titan.dds'},
							{label = 'Typhoon', value = 'soul_typhoon.dds'},
							{label = 'Maelstrom', value = 'soul_maelstrom.dds'}}
			}
		}
	}
end